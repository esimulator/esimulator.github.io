...
function updateDate(dates) {
    date = dates;
    $('.current_date').html(Date.parse(dates).toString('MMM, yyyy'));
}
...
function updateLeftBarProjects() {
    ...
    projectsArray.forEach(function(item) {
        ...
        if (item.status == "progress") {
            var yearsLeft = item.startDate.getMonth() + item.terms - date.getMonth();
            ...
            if (yearsLeft != 0) {
                ...
                div.append(projectTitle);
                    ...
                    var days = $("<p>", {class: "days", html: yearsLeft + " month left"});
